...
public class Homework4Test {
    ...
    public void compareLinkedLists(LinkedList res, ArrayList<Integer> expected) {
        Iterator<Node> it1 = res.iterator();
        Iterator<Integer> expectedIt = expected.iterator();
        if (!it1.hasNext())
            Assert.fail("No elements in the resulting list");
        while (expectedIt.hasNext()) {
            if (!it1.hasNext())
                Assert.fail("Not enough elements in the resulting list");
            Node n = it1.next();
            // System.out.println(n.elem());
            int elem = n.elem();
            int expectedElem = expectedIt.next();
            Assert.assertEquals("The element in the expected list does not match what your sublist method produces",
                    expectedElem, elem);
        }
        if (it1.hasNext()) {
            // resulting list has more elements than expected
            Assert.fail("More elements in the resulting list than expected");
